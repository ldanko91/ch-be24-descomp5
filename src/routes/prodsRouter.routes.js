...
prodsRouter.get('/:pcod' , async (req, res) => {
    let { pcod } = req.params
    let productos = await DBProductsController.getProductByCode(pcod);
    console.log(productos)
    res.send({statyus: "success", payload:productos})
})

prodsRouter.post('/backoffice',  async (req, res) => {
...
prodsRouter.delete('/backoffice/:pcod',  async (req, res) => {
    let { pcod } = req.params
